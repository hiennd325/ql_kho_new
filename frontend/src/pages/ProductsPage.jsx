    ...
    <div className="space-y-6">
      ...
      <div className={`flex flex-col xl:flex-row xl:items-center justify-between gap-6 sm:gap-8 p-5 sm:p-8 rounded-[24px] sm:rounded-[32px] border shadow-[0_8px_30px_rgb(0,0,0,0.02)] mb-6 sm:mb-8 transition-colors duration-300 ${isDarkMode ? 'bg-slate-900/80 backdrop-blur-2xl border border-white/10 shadow-2xl shadow-black/40' : 'bg-white/85 backdrop-blur-2xl border border-white/60 shadow-2xl shadow-slate-200/50'}`}>
        <div className="flex items-center gap-4 sm:gap-6">
          <div className="bg-gradient-to-br from-[#000000] to-[#000000] p-3 sm:p-5 rounded-[18px] sm:rounded-[24px] text-white shadow-xl shadow-[#000000]/20">
        ...
        <div className="flex flex-wrap items-center gap-3 sm:gap-4">
          <button
            ...
            className={`flex items-center gap-2 sm:gap-3 px-5 sm:px-8 py-3 sm:py-4 rounded-2xl hover:bg-[#000000] transition-all font-black shadow-xl active:scale-95 group uppercase tracking-widest text-[10px] sm:text-xs ${isDarkMode ? 'bg-[#000000] text-white shadow-[#000000]/20' : 'bg-[#000000] text-white shadow-[#000000]/10'}`}
          >
          ...
          <div className="flex items-center gap-2">
            <button onClick={exportToCSV} className={`p-3 sm:p-4 rounded-2xl hover:bg-white hover:shadow-lg transition-all hover:text-[#000000] group border border-transparent hover:border-slate-100 ${isDarkMode ? 'bg-slate-800 text-slate-400 border-slate-700' : 'bg-slate-50 text-slate-600'}`} title="Xuất CSV">
      ...
      <div className={`p-4 sm:p-5 rounded-2xl shadow-sm border flex flex-col sm:flex-row gap-4 sm:gap-5 items-stretch sm:items-center transition-colors duration-300 ${isDarkMode ? 'bg-slate-900/80 backdrop-blur-2xl border border-white/10 shadow-2xl shadow-black/40' : 'bg-white/85 backdrop-blur-2xl border border-white/60 shadow-2xl shadow-slate-200/50'}`}>
        <div className="relative group flex-1">
          ...
          <input
            ...
            className={`w-full pl-12 pr-4 py-3.5 border rounded-xl outline-none focus:ring-2 focus:ring-[#000000] transition-all font-medium ${isDarkMode ? 'bg-slate-800 border-slate-700 text-white focus:bg-slate-700' : 'bg-slate-50 border-slate-200 text-slate-900 focus:bg-white'}`}
            value={searchTerm}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className={`divide-y ${isDarkMode ? 'divide-slate-800' : 'divide-slate-50'}`}>
              {loading ? (
                <tr><td colSpan="6" className="px-8 py-24 text-center"><div className="flex flex-col items-center gap-4"><RefreshCw className="w-10 h-10 text-[#000000] animate-spin" /><p className="text-slate-400 font-black uppercase text-xs tracking-widest animate-pulse">Đang tải dữ liệu...</p></div></td></tr>
              ) : products.length === 0 ? (
                ...
                products.map((product) => (
                  <tr key={product.id} className={`transition-all group border-b ${isDarkMode ? 'border-slate-800/60 hover:bg-[#000000]/5 hover:border-[#000000]/20' : 'border-slate-100 hover:bg-[#000000]/5 hover:border-[#000000]/20'}`}>
                    <td className="px-8 py-6">
                      <div className="flex flex-col">
                        <span className={`text-sm font-black group-hover:text-[#000000] transition-colors ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{product.name}</span>
                    ...
                    <td className="px-8 py-6">
                      <div className="flex justify-center items-center gap-3">
                        <button onClick={() => openDetailModal(product)} className={`p-3 text-slate-400 border hover:border-[#000000] hover:text-[#000000] hover:shadow-lg hover:shadow-[#000000]/10 rounded-2xl transition-all active:scale-90 ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white/85 backdrop-blur-2xl border border-white/60 shadow-2xl shadow-slate-200/50'}`} title="Chi tiết">
                          <Eye size={16} strokeWidth={2.5} />
                        </button>
                        <button onClick={() => openEditModal(product)} className={`p-3 text-slate-400 border hover:border-[#000000] hover:text-[#000000] hover:shadow-lg hover:shadow-[#000000]/10 rounded-2xl transition-all active:scale-90 ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white/85 backdrop-blur-2xl border border-white/60 shadow-2xl shadow-slate-200/50'}`} title="Sửa">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className={`rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden border transition-all ${isDarkMode ? 'bg-slate-900/80 backdrop-blur-2xl border border-white/10 shadow-2xl shadow-black/40' : 'bg-white/85 backdrop-blur-2xl border border-white/60 shadow-2xl shadow-slate-200/50'}`}>
            <div className={`flex items-center justify-between p-6 border-b ${isDarkMode ? 'border-slate-800' : 'border-slate-100'}`}>
              <div className="flex items-center gap-3">
                <div className="bg-[#000000] p-2 rounded-xl text-white">
            ...
            <form onSubmit={handleAddSubmit} className="p-8 space-y-6">
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                <button type="submit" className="px-10 py-3 bg-[#000000] text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-[#000000] shadow-xl shadow-[#000000]/20 transition-all active:scale-95">Xác nhận thêm</button>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className={`rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden border transition-all ${isDarkMode ? 'bg-slate-900/80 backdrop-blur-2xl border border-white/10 shadow-2xl shadow-black/40' : 'bg-white/85 backdrop-blur-2xl border border-white/60 shadow-2xl shadow-slate-200/50'}`}>
            ...
            <form onSubmit={handleEditSubmit} className="p-8 space-y-6">
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                <button type="submit" className="px-10 py-3 bg-[#000000] text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-[#333333] shadow-xl shadow-[#000000]/20 transition-all active:scale-95">Lưu thay đổi</button>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className={`rounded-[32px] shadow-2xl w-full max-w-2xl overflow-hidden border transition-all ${isDarkMode ? 'bg-slate-900/80 backdrop-blur-2xl border border-white/10 shadow-2xl shadow-black/40' : 'bg-white/85 backdrop-blur-2xl border border-white/60 shadow-2xl shadow-slate-200/50'}`}>
            ...
            <div className="p-10">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <p className="text-3xl font-black text-[#000000] tracking-tighter tabular-nums">{formatCurrency(selectedProduct.price)}</p>
                  </div>
                </div>
                <div className="space-y-6">
                  <div>
              ...
              <div className="flex justify-end mt-10">
                <button onClick={() => setIsDetailModalOpen(false)} className={`px-10 py-4 rounded-xl text-xs font-black uppercase tracking-widest transition-all active:scale-95 ${isDarkMode ? 'bg-[#000000] text-white hover:bg-[#000000] shadow-xl shadow-[#000000]/20' : 'bg-[#000000] text-white hover:bg-[#000000] shadow-xl shadow-[#000000]/10'}`}>Đóng cửa sổ</button>
              </div>
            </div>
          </div>
        </div>
