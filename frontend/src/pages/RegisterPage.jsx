        ...
        <div className={`backdrop-blur-xl border rounded-[2.5rem] shadow-2xl p-10 overflow-hidden relative group transition-colors duration-300 ${isDarkMode ? 'bg-white/[0.03] border-white/10' : 'bg-white border-slate-200'}`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {[
              ...
              <div key={label} className="space-y-2">
                <label className="text-[11px] font-black text-[#FF5E3A] uppercase tracking-[0.2em] ml-2">{label}</label>
                <div className="relative group">
                  ...
                  <input
                    ...
                    className={`w-full border rounded-2xl pl-12 pr-4 py-3.5 focus:ring-2 focus:ring-[#FF5E3A]/50 focus:border-[#FF5E3A] outline-none transition-all font-medium placeholder:text-gray-600 ${isDarkMode ? 'bg-white/[0.05] border-white/10 text-white focus:bg-white/[0.08]' : 'bg-slate-50 border-slate-200 text-slate-900 focus:bg-white'}`}
          ...
          <div className="mt-8 text-center">
            <p className={`${isDarkMode ? 'text-gray-500' : 'text-slate-500'} text-sm font-medium`}>
              Đã có tài khoản?{' '}
              <Link to="/login" className="text-[#FF5E3A] font-bold hover:text-[#e04520] transition-colors decoration-2 underline-offset-4">
                Đăng nhập ngay
              </Link>
            </p>
          </div>
