      ...
      <div className="relative z-20 flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-white/85 dark:bg-slate-900/80 backdrop-blur-2xl shadow-2xl border dark:border-white/10 border-white/60 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
        <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-lg w-full lg:w-fit overflow-x-auto scrollbar-hide">
          <button
            ...
            className={`flex-1 lg:flex-none px-4 sm:px-6 py-2 rounded-md text-xs sm:text-sm font-bold transition-all whitespace-nowrap ${activeTab === 'import' ? 'bg-white dark:bg-slate-700 text-[#000000] dark:text-[#ffffff] shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}
          >
            Phiếu nhập
          </button>
          <button
            onClick={() => { setActiveTab('export'); setCurrentPage(1); }}
            className={`flex-1 lg:flex-none px-4 sm:px-6 py-2 rounded-md text-xs sm:text-sm font-bold transition-all whitespace-nowrap ${activeTab === 'export' ? 'bg-white dark:bg-slate-700 text-[#000000] dark:text-[#ffffff] shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}
          >
            Phiếu xuất
          </button>
          <button
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          <div className="relative group flex-1 sm:min-w-[200px] lg:min-w-[240px]">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-[#000000] transition-colors" size={18} />
            <input
              type="text"
              placeholder="Tìm kiếm..."
              className="pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:ring-2 focus:ring-[#000000] focus:bg-white dark:focus:bg-slate-700 w-full transition-all text-sm font-medium text-slate-900 dark:text-slate-100"
              value={searchTerm}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 bg-[#000000] text-white px-4 py-2.5 rounded-xl hover:bg-[#000000] transition-all text-xs font-black shadow-lg shadow-[#000000]/10 dark:shadow-[#000000]/20 active:scale-95"
              >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {loading ? (
                <tr><td colSpan="7" className="px-4 py-20 text-center"><div className="flex flex-col items-center gap-3"><div className="w-10 h-10 border-4 border-[#000000] border-t-transparent rounded-full animate-spin"></div><p className="text-slate-400 dark:text-slate-500 font-bold italic">Đang tải dữ liệu...</p></div></td></tr>
              ) : activeTab === 'alerts' ? (
                alerts.length === 0 ? (
                    ...
                    <tr key={alert.id} className="hover:bg-rose-50/30 dark:hover:bg-rose-900/10 transition-colors group">
                      <td className="px-4 sm:px-8 py-5">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
                          ...
                          <span className="text-xs sm:text-sm text-slate-700 dark:text-slate-300 font-bold group-hover:text-[#000000] dark:group-hover:text-[#ffffff] transition-colors">{alert.name}</span>
                ...
                Object.entries(groupedTransactions).map(([refId, group]) => (
                  <React.Fragment key={refId}>
                    {group.map((t, idx) => {
                      ...
                      return (
                        <tr key={idx} className={`border-b transition-all ${isDarkMode ? 'border-slate-800/60 hover:bg-[#000000]/5 hover:border-[#000000]/20' : 'border-slate-100 hover:bg-[#000000]/5 hover:border-[#000000]/20'} ${idx > 0 ? 'bg-slate-50/20 dark:bg-slate-800/10' : ''}`}>
                          {idx === 0 && (
                            <>
                              <td className="px-4 sm:px-8 py-5 text-sm font-black text-[#000000] dark:text-[#ffffff]" rowSpan={expandedGroups[refId] ? group.length : 1}>
                                 <div className="flex flex-col xs:flex-row xs:items-center gap-2 xs:gap-3">
                                   <div className="bg-[#000000]/8 dark:bg-[#000000]/10 px-2 sm:px-3 py-1 rounded-lg w-fit border border-[#000000]/15 dark:border-[#000000]/30 font-mono text-xs sm:text-sm">{refId}</div>
                                   {group.length > 1 && (
                                     <button
                                       onClick={() => toggleGroup(refId)}
                                       className="p-1 hover:bg-[#000000]/12 dark:hover:bg-[#000000]/15 rounded-md transition-colors text-[#000000] w-fit"
                                     >
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center bg-white/85 dark:bg-slate-800/80 backdrop-blur-2xl shadow-2xl border dark:border-white/10 border-white/60 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm overflow-hidden">
                <input
                  ...
                  className="w-12 py-2 text-center text-sm font-black text-[#000000] dark:text-[#ffffff] bg-transparent outline-none"
                />
