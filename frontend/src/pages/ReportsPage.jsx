    ...
    <div className="space-y-6">
      ...
      <div className="flex bg-white/85 dark:bg-slate-900/80 backdrop-blur-2xl shadow-2xl border dark:border-white/10 border-white/60 p-1 rounded-xl border border-gray-200 dark:border-slate-800 w-full sm:w-fit shadow-sm overflow-x-auto scrollbar-hide">
        <button
          ...
          className={`flex-1 sm:flex-none px-4 sm:px-6 py-2.5 rounded-lg text-xs sm:text-sm font-bold flex items-center justify-center gap-2 transition-all whitespace-nowrap ${activeTab === 'audits' ? 'bg-[#000000] text-white shadow-md shadow-[#000000]/20' : 'text-gray-600 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-800'}`}
        >
          <ClipboardList size={16} /> <span className="hidden xs:inline">Phiếu</span> Kiểm kê
        </button>
        <button
          onClick={() => setActiveTab('inventory')}
          className={`flex-1 sm:flex-none px-4 sm:px-6 py-2.5 rounded-lg text-xs sm:text-sm font-bold flex items-center justify-center gap-2 transition-all whitespace-nowrap ${activeTab === 'inventory' ? 'bg-[#000000] text-white shadow-md shadow-[#000000]/20' : 'text-gray-600 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-800'}`}
        >
      ...
      <div className="bg-white/85 dark:bg-slate-900/80 backdrop-blur-2xl shadow-2xl border dark:border-white/10 border-white/60 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800 flex flex-wrap gap-4 items-end">
          <div className="flex-1 min-w-[200px] space-y-1">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 bg-white/85 dark:bg-slate-800/80 backdrop-blur-2xl shadow-2xl border dark:border-white/10 border-white/60 border border-gray-200 dark:border-slate-700 rounded-lg outline-none focus:ring-2 focus:ring-[#000000] text-sm text-slate-900 dark:text-slate-100 transition-all"
                value={searchTerm}
          ...
          <div className="space-y-1">
            ...
            <select
              className="border border-gray-200 dark:border-slate-700 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-[#000000] text-sm bg-white/85 dark:bg-slate-800/80 backdrop-blur-2xl shadow-2xl border dark:border-white/10 border-white/60 text-slate-700 dark:text-slate-200 min-w-[150px] transition-all"
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="border border-gray-200 dark:border-slate-700 rounded-lg px-3 py-1.5 text-sm outline-none focus:ring-2 focus:ring-[#000000] bg-white/85 dark:bg-slate-800/80 backdrop-blur-2xl shadow-2xl border dark:border-white/10 border-white/60 text-slate-700 dark:text-slate-200 transition-all"
          ...
          <div className="flex gap-2 ml-auto">
            {activeTab === 'audits' && (
              <button onClick={openAddAudit} className="bg-[#000000] text-white px-4 py-2 rounded-lg hover:bg-[#000000] font-bold text-sm flex items-center gap-2 shadow-md shadow-[#000000]/10 dark:shadow-[#000000]/20 active:scale-95 transition-all">
                  ...
                  {audits.length === 0 ? (
                    ...
                    audits.map(audit => (
                      <tr key={audit.id} className="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors group">
                        <td className="px-6 py-4 text-sm font-bold text-[#000000] dark:text-[#ffffff]">{audit.code}</td>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => downloadAuditPDF(audit.id)} className="p-1.5 text-[#000000] dark:text-[#ffffff] hover:bg-[#000000]/8 dark:hover:bg-[#000000]/10 rounded transition-all" title="Tải PDF"><Download size={16} /></button>
                ...
                {inventoryReport.length === 0 ? (
                  ...
                  inventoryReport.map(item => (
                    <tr key={item.product_id} className="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-right font-bold text-[#000000] dark:text-[#ffffff]">{item.quantity}</td>
      ...
      {isAuditModalOpen && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          <div className="bg-white/85 dark:bg-slate-900/80 backdrop-blur-2xl shadow-2xl border dark:border-white/10 border-white/60 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col animate-in fade-in zoom-in duration-200 border dark:border-slate-800">
            <div className="flex items-center justify-between p-6 border-b dark:border-slate-800 bg-[#000000]/8/50 dark:bg-[#000000]/10">
              <h3 className="text-xl font-bold text-[#000000] dark:text-[#ffffff] flex items-center gap-2">
            ...
            <form onSubmit={handleAuditSubmit} className="flex-1 overflow-hidden flex flex-col">
              <div className="p-6 overflow-y-auto space-y-6 bg-white/85 dark:bg-slate-900/80 backdrop-blur-2xl shadow-2xl border dark:border-white/10 border-white/60">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between border-b dark:border-slate-800 pb-2">
                    <h4 className="font-bold text-gray-800 dark:text-slate-100 flex items-center gap-2"><Package size={18} className="text-[#000000] dark:text-[#ffffff]" /> Chi tiết hàng hóa</h4>
                    <div className="flex gap-2">
                      <select
                        className="text-sm border border-gray-300 dark:border-slate-700 rounded-lg px-3 py-1.5 outline-none focus:ring-2 focus:ring-[#000000] bg-white/85 dark:bg-slate-800/80 backdrop-blur-2xl shadow-2xl border dark:border-white/10 border-white/60 text-slate-900 dark:text-slate-100 min-w-[200px] transition-all"
                          ...
                          auditForm.items.map((item, idx) => (
                            <tr key={item.product_id} className="hover:bg-gray-50/50 dark:hover:bg-slate-800 transition-colors">
                              ...
                              <td className="px-4 py-3 text-center">
                                <input
                                  type="number" min="0"
                                  className="w-20 px-2 py-1.5 bg-white/85 dark:bg-slate-800/80 backdrop-blur-2xl shadow-2xl border dark:border-white/10 border-white/60 border border-gray-300 dark:border-slate-700 rounded-lg text-center font-bold focus:ring-2 focus:ring-[#000000] outline-none text-slate-900 dark:text-slate-100 transition-all"
              ...
              <div className="p-6 border-t dark:border-slate-800 bg-gray-50 dark:bg-slate-800/50 flex justify-end gap-3">
                ...
                <button type="submit" className="px-10 py-2.5 bg-[#000000] text-white rounded-xl hover:bg-[#000000] font-bold shadow-lg shadow-[#000000]/10 dark:shadow-[#000000]/20 transition-all active:scale-95 flex items-center gap-2">
